  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor("status", {
      ...
      cell: (info) => {
        ...
        const color = val === "정상" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700";
        return <span className={`${color} px-2 py-0.5 rounded-full text-xs font-bold`}>{val}</span>;
      },
    }),
    ...
    columnHelper.accessor("repairCost", {
      header: "유지보수비",
      cell: (info) => <span className="text-blue-600 font-mono">₩ {info.getValue().toLocaleString()}</span>,
    }),
